...
test('Change title when first content is divider', async ({ page }) => {
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/Alan-Cusack/blockexpanse/issues/1004',
  });
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
  await focusRichText(page);
...
test('the cursor should move to closest editor block when clicking outside container', async ({
  ...
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/Alan-Cusack/blockexpanse/pull/570',
  });
...
test('should not scroll page when mouse is click down', async ({ page }) => {
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/Alan-Cusack/blockexpanse/issues/5034',
  });
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
  await focusRichText(page);
...
test('click to select divided', async ({ page }) => {
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/Alan-Cusack/blockexpanse/issues/4547',
  });
...
test('abnormal cursor jumping', async ({ page }) => {
  // https://github.com/Alan-Cusack/blockexpanse/pull/8552
...
test('unexpected scroll when clicking padding area', async ({ page }) => {
  // https://github.com/Alan-Cusack/blockexpanse/pull/8678
