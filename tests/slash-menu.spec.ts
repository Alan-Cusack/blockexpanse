...
test('should clean slash string after soft enter', async ({ page }) => {
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/Alan-Cusack/blockexpanse/issues/1126',
  });
  await enterPlaygroundRoom(page);
...
test.describe('slash menu with customize menu', () => {
  test('can remove specified menus', async ({ page }) => {
    ...
    await page.evaluate(async () => {
      ...
      const SlashMenuWidget = window.$blockexpanse.blocks.AffineSlashMenuWidget;
      ...
      const pageSpecs = window.$blockexpanse.blocks.PageEditorBlockSpecs;
      editor.pageSpecs = [
        ...pageSpecs,
        {
          setup: di => {
            di.override(
              window.$blockexpanse.identifiers.WidgetViewMapIdentifier(
                'affine:page'
              ),
              // @ts-ignore
              () => ({
