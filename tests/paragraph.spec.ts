...
test('init paragraph by page title enter at last', async ({ page }) => {
  ...
  //#region Fixes: https://github.com/Alan-Cusack/blockexpanse/issues/1007
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/Alan-Cusack/blockexpanse/issues/1007',
  });
...
test('arrow up/down navigation within and across paragraphs containing different types of text', async ({
  ...
  test.info().annotations.push({
    type: 'issue',
    description: 'https://github.com/Alan-Cusack/blockexpanse/issues/5155',
  });
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
  await focusRichText(page);
