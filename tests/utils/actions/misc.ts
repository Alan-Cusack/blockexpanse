...
import type { BlockExpanseFlags } from '@blockexpanse/global/types';
...
declare global {
  interface WindowEventMap {
    'blockexpanse:doc-ready': CustomEvent<string>;
  }
}
...
async function initEmptyEditor({
  ...
  page: Page;
  flags?: Partial<BlockExpanseFlags>;
  noInit?: boolean;
  multiEditor?: boolean;
}) {
  await page.evaluate(
    ([flags, noInit, multiEditor]) => {
      ...
      async function waitForMountPageEditor(
        ...
        // add app root from https://github.com/Alan-Cusack/blockexpanse/commit/947201981daa64c5ceeca5fd549460c34e2dabfa
        ...
        const createEditor = () => {
          ...
          const defaultExtensions: ExtensionType[] = [
            ...window.$blockexpanse.defaultExtensions(),
            {
              setup: di => {
                di.addImpl(
                  window.$blockexpanse.identifiers.ParseDocUrlService,
                  {
                    parseDocUrl() {
                      return undefined;
                    },
            ...
            },
            {
              setup: di => {
                di.override(
                  window.$blockexpanse.identifiers.DocModeProvider,
                  window.$blockexpanse.mockServices.mockDocModeService(
                    () => editor.mode,
          ...
          editor.std
            .get(window.$blockexpanse.identifiers.RefNodeSlotsProvider)
        ...
        editor.updateComplete
          .then(() => {
            ...
            window.dispatchEvent(
              new CustomEvent('blockexpanse:doc-ready', { detail: doc.id })
            );
          })
          .catch(console.error);
      }
...
export async function enterPlaygroundRoom(
  ...
  await page.evaluate(() => {
    if (typeof window.$blockexpanse !== 'object') {
      throw new Error('window.$blockexpanse is not object');
    }
  }, []);
  return room;
}
...
export async function getClipboardCustomData(page: Page, type: string) {
  const dataInClipboard = await page.evaluate(async () => {
    ...
    return container?.dataset.blockexpanseSnapshot ?? '';
  });
