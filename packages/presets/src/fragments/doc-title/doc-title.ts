...
import type { RichText, RootBlockModel } from '@blockexpanse/blocks';
...
export class DocTitle extends WithDisposable(ShadowlessElement) {
  ...
  private _resolveI18n() {
    // `doc-title` is rendered as a sibling of `editor-host`, not inside it.
    if (this.std) {
      return this.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    }
    const host =
      (this.closest('.affine-page-viewport')?.querySelector(
        'editor-host'
      ) as EditorHost | null) ??
  ...
  /**
   * Optional std scope. Preferred over DOM lookup because `doc-title`
   * lives outside `editor-host` in the page viewport.
   */
  @property({ attribute: false })
  accessor std: EditorHost['std'] | null = null;
}
