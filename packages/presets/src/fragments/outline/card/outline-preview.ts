...
export class OutlineBlockPreview extends SignalWatcher(
    ...
    const previewText = deltas.map(delta => {
      if (delta.attributes?.reference) {
        ...
        const title = unavailable
          ? t(I18nKeys.editor.common.deletedDoc, 'Deleted doc')
          : refMeta.title;
        return html`<span
          ...
          <span class="linked-doc-text"
            >${title.length
              ? title
              : t(I18nKeys.editor.common.untitled, 'Untitled')}</span
          ></span
        >`;
      } else {
  ...
  renderBlockByFlavour() {
    ...
    switch (block.flavour as keyof BlockExpanse.BlockModels) {
      case 'affine:page':
  ...
  accessor block!: ValuesOf<BlockExpanse.BlockModels>;
