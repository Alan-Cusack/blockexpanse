...
import { DisposableGroup, WithDisposable } from '@blockexpanse/global/utils';
import { css, html, LitElement, type PropertyValues } from 'lit';
import { property, query, state } from 'lit/decorators.js';

import { SettingsIcon, SmallFrameNavigatorIcon } from '../../_common/icons.js';

function resolveI18nFn(editorHost?: EditorHost): I18nFn {
  if (editorHost) {
    return editorHost.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
  }
    ...
    return html`<div class="frame-panel-header">
      <div class="all-frames-setting">
        ...
        <edgeless-tool-icon-button
          ...
          .tooltip=${this._settingPopperShow
            ? ''
            : t(
                I18nKeys.editor.framePanel.allFramesSettings,
                'All Frames Settings'
              )}
          .tipPosition=${'top'}
      ...
      <div class="presentation-button" @click=${this._enterPresentationMode}>
        ${SmallFrameNavigatorIcon}<span class="presentation-button-label"
          >${t(I18nKeys.editor.framePanel.presentation, 'Presentation')}</span
        >
      </div>
    </div>`;
  }
