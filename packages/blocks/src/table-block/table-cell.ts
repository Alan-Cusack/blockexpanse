import type { TableColumn, TableRow } from '@blockexpanse/affine-model';
...
import { TextBackgroundDuotoneIcon } from '@blockexpanse/affine-components/icons';
import {
...
import type { TableDataManager } from './table-data-manager.js';

import { colorList } from './color.js';
import { ColumnMaxWidth, DefaultColumnWidth } from './consts.js';
import {
...
export class TableCell extends SignalWatcher(
  ...
  private get t(): I18nFn {
    const table = this.closest<TableBlockComponent>('affine-table');
    return table?.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
  }
  ...
  private colorLabel(id: string | undefined): string {
    if (!id) {
      return this.t(I18nKeys.editor.table.default, 'Default');
    }
    const key =
      I18nKeys.editor.format[id as keyof typeof I18nKeys.editor.format];
    return key ? this.t(key, id) : id;
  }

  private colorMenuItems(
    ...
    return [
      ...
      menu.action({
        prefix: html`<div
          ...
          tableTheme.layerPrimary};display: flex;align-items: center;justify-content: center;"
        >
          ${TextBackgroundDuotoneIcon}
        </div>`,
        name: this.colorLabel(item.id),
        isSelected: currentColor === item.color,
        select: () => select(item.color),
      })
    );
  }
  ...
  createColorPickerMenu(
    ...
    return menu.subMenu({
      name: this.t(I18nKeys.editor.table.backgroundColor, 'Background color'),
      prefix: ColorPickerIcon(),
      options: {
        items: this.colorMenuItems(currentColor, select),
      },
    });
  }
        ...
        options: {
          items: [
            menu.group({
              items: [
                menu.action({
                  name: this.t(I18nKeys.editor.table.copy, 'Copy'),
                  prefix: CopyIcon(),
                  select: () => {
            ...
            menu.group({
              items: [
                menu.action({
                  name: this.t(
                    I18nKeys.editor.table.clearContents,
                    'Clear contents'
                  ),
                  prefix: CloseIcon(),
                  select: () => {
        ...
        items: [
          menu.group({
            items: [
              menu.subMenu({
                name: this.t(
                  I18nKeys.editor.table.backgroundColor,
                  'Background color'
                ),
                prefix: ColorPickerIcon(),
                options: {
                  items: this.colorMenuItems(column.backgroundColor, color => {
              ...
              ...(column.backgroundColor
                ? [
                    menu.action({
                      name: this.t(
                        I18nKeys.editor.table.clearColumnStyle,
                        'Clear column style'
                      ),
                      prefix: CloseIcon(),
                      select: () => {
          ...
          menu.group({
            items: [
              menu.action({
                name: this.t(I18nKeys.editor.table.insertLeft, 'Insert Left'),
              ...
              menu.action({
                name: this.t(I18nKeys.editor.table.insertRight, 'Insert Right'),
              ...
              menu.action({
                name: this.t(I18nKeys.editor.table.moveLeft, 'Move Left'),
              ...
              menu.action({
                name: this.t(I18nKeys.editor.table.moveRight, 'Move Right'),
          ...
          menu.group({
            items: [
              menu.action({
                name: this.t(I18nKeys.editor.table.duplicate, 'Duplicate'),
              ...
              menu.action({
                name: this.t(
                  I18nKeys.editor.table.clearColumnContents,
              ...
              menu.action({
                name: this.t(I18nKeys.editor.table.delete, 'Delete'),
                class: {
                  'delete-item': true,
                },
                prefix: DeleteIcon(),
      ...
      options: {
        ...
        items: [
          menu.group({
            items: [
              menu.subMenu({
                ...
                prefix: ColorPickerIcon(),
                options: {
                  items: this.colorMenuItems(row.backgroundColor, color => {
                    this.dataManager.setRowBackgroundColor(row.rowId, color);
                  }),
                },
              }),
              ...(row.backgroundColor
                ? [
                    menu.action({
                      name: this.t(
                        I18nKeys.editor.table.clearRowStyle,
                        'Clear row style'
                      ),
                      prefix: CloseIcon(),
                      select: () => {
          ...
          menu.group({
            items: [
              menu.action({
                name: this.t(I18nKeys.editor.table.insertAbove, 'Insert Above'),
                prefix: InsertTopIcon(),
                select: () => {
              ...
              menu.action({
                name: this.t(I18nKeys.editor.table.insertBelow, 'Insert Below'),
                prefix: InsertBleowIcon(),
                select: () => {
                  this.dataManager.insertRow(rowIndex);
                },
              }),
              menu.action({
                name: this.t(I18nKeys.editor.table.moveUp, 'Move Up'),
              ...
              menu.action({
                name: this.t(I18nKeys.editor.table.moveDown, 'Move Down'),
          ...
          menu.group({
            items: [
              ...
              menu.action({
                name: this.t(
                  I18nKeys.editor.table.clearRowContents,
                  'Clear row contents'
                ),
                prefix: CloseIcon(),
                select: () => {
...
export const createColumnDragPreview = (cells: TableCell[]) => {
  ...
  container.style.backgroundColor = tableTheme.layerPrimary;
  cells.forEach((cell, index) => {
    const div = document.createElement('div');
    const td = cell.querySelector('td');
    if (index !== 0) {
      div.style.borderTop = `1px solid ${tableTheme.layerInsideBorder}`;
    }
    if (td) {
...
export const createRowDragPreview = (cells: TableCell[]) => {
  ...
  cells.forEach((cell, index) => {
    ...
    const td = cell.querySelector('td');
    if (index !== 0) {
      div.style.borderLeft = `1px solid ${tableTheme.layerInsideBorder}`;
    }
    if (td) {
