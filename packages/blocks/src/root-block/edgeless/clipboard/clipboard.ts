import type { Connection } from '@blockexpanse/affine-model';
import type {
  BlockStdScope,
  EditorHost,
  SurfaceSelection,
...
import type { IBound, IVec, SerializedXYWH } from '@blockexpanse/global/utils';
...
const BLOCKEXPANSE_SURFACE = 'blockexpanse/surface';
...
export class EdgelessClipboardController extends PageClipboard {
  ...
  private _onCopy = async (
    ...
    await this.std.clipboard.writeToClipboard(async _items => {
      ...
      return {
        ..._items,
        [BLOCKEXPANSE_SURFACE]: JSON.stringify(data),
      };
    });
  };
  ...
  private _onPaste = async (_context: UIEventStateContext) => {
    ...
    if (isUrlInClipboard(data)) {
      ...
      if (docUrlInfo) {
        ...
        if (embedOptions) {
          flavour = embedOptions.flavour as BlockExpanse.EdgelessModelKeys;
    ...
    try {
      ...
      const mayBeSurfaceDataJson = json[BLOCKEXPANSE_SURFACE];
  ...
  private async _edgelessToCanvas(
    ...
    bound: IBound,
    nodes?: BlockExpanse.EdgelessBlockModelType[],
    canvasElements: BlockExpanse.SurfaceModel[] = [],
    {
      background,
    ...
    const _drawTopLevelBlock = async (
      block: BlockExpanse.EdgelessBlockModelType,
      isInFrame = false
    ) => {
    ...
    const nodeElements =
      ...
      (edgeless.service.gfx.getElementsByBound(bound, {
        type: 'block',
      }) as BlockExpanse.EdgelessBlockModelType[]);
    for (const nodeElement of nodeElements) {
      await _drawTopLevelBlock(nodeElement);

      if (matchFlavours(nodeElement, ['affine:frame'])) {
        const blocksInsideFrame: BlockExpanse.EdgelessBlockModelType[] = [];
        this.edgeless.service.frame
          .getElementsInFrameBound(nodeElement, false)
          .forEach(ele => {
            if (isTopLevelBlock(ele)) {
              blocksInsideFrame.push(
                ele as BlockExpanse.EdgelessBlockModelType
              );
            } else {
              canvasElements.push(ele as BlockExpanse.SurfaceModel);
            }
          });
  ...
  private _updatePastedElementsIndex(
    ...
    function compare(
      a: BlockExpanse.EdgelessModel,
      b: BlockExpanse.EdgelessModel
    ) {
      if (a instanceof SurfaceGroupLikeModel && a.hasDescendant(b)) {
        ...
        const aGroups = a.groups as BlockExpanse.SurfaceGroupLikeModel[];
        const bGroups = b.groups as BlockExpanse.SurfaceGroupLikeModel[];

        let i = 1;
        let aGroup: BlockExpanse.EdgelessModel | undefined = nToLast(
          aGroups,
          i
        );
        let bGroup: BlockExpanse.EdgelessModel | undefined = nToLast(
          bGroups,
          i
        );
  ...
  async copyAsPng(
    blocks: BlockExpanse.EdgelessBlockModelType[],
    shapes: BlockExpanse.SurfaceModel[]
  ) {
  ...
  async createElementsFromClipboardData(
    ...
    const blockModels: BlockExpanse.EdgelessBlockModelType[] = [];
    const canvasElements: BlockExpanse.SurfaceModel[] = [];
    const allElements: BlockExpanse.EdgelessModel[] = [];
    ...
    for (const data of elementsRawData) {
      ...
      if (blockSnapshot) {
        ...
        assertType<BlockExpanse.EdgelessBlockModelType>(block.model);
...
export async function prepareClipboardData(
  selectedAll: BlockExpanse.EdgelessModel[],
  std: BlockStdScope
) {
  const job = new Job({
