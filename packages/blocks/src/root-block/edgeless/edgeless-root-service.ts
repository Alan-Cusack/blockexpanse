...
import { clamp } from '@blockexpanse/affine-shared/utils';
import {
...
import { Bound, getCommonBound } from '@blockexpanse/global/utils';
import { type BlockModel, Slot } from '@blockexpanse/store';
...
export class EdgelessRootService extends RootService implements SurfaceContext {
  ...
  updateElement = (id: string, props: Record<string, unknown>) => {
    ...
    if (element) {
      const key = getLastPropsKey(
        element.type as BlockExpanse.EdgelessModelKeys,
    ...
    if (block) {
      const key = getLastPropsKey(
        block.flavour as BlockExpanse.EdgelessModelKeys,
  ...
  addBlock(
    ...
    const key = getLastPropsKey(
      flavour as BlockExpanse.EdgelessModelKeys,
      props
    );
    if (key) {
  ...
  addElement<T extends Record<string, unknown>>(type: string, props: T) {
    const key = getLastPropsKey(type as BlockExpanse.EdgelessModelKeys, props);
    if (key) {
  ...
  createGroup(elements: BlockExpanse.EdgelessModel[] | string[]) {
  ...
  getConnectors(element: BlockExpanse.EdgelessModel | string) {
    const id = typeof element === 'string' ? element : element.id;

    return this.surface.getConnectors(id) as ConnectorElementModel[];
  }

  getElementById(id: string): BlockExpanse.EdgelessModel | null {
    const el =
      this._surface.getElementById(id) ??
      (this.doc.getBlockById(id) as BlockExpanse.EdgelessBlockModelType | null);
    return el;
  }

  getElementsByType<K extends keyof BlockExpanse.SurfaceElementModelMap>(
    type: K
  ): BlockExpanse.SurfaceElementModelMap[K][] {
  ...
  ): BlockExpanse.EdgelessModel | null {
    return this.gfx.getElementInGroup(x, y, options);
  }

  removeElement(id: string | BlockExpanse.EdgelessModel) {
  ...
  reorderElement(
    element: BlockExpanse.EdgelessModel,
