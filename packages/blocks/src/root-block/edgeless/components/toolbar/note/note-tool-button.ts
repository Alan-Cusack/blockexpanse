...
import { ArrowUpIcon, NoteIcon } from '@blockexpanse/affine-components/icons';
import {
  I18nKeys,
  I18nProvider,
  identityI18nFn,
...
export class EdgelessNoteToolButton extends QuickToolMixin(LitElement) {
  ...
  override render() {
    ...
    const noteLabel = t(I18nKeys.editor.edgeless.note, 'Note');
    return html`
      <edgeless-tool-icon-button
        class="edgeless-note-button"
        .tooltip=${this._noteMenu ? '' : getTooltipWithShortcut(noteLabel, 'N')}
        .tooltipOffset=${17}
        .active=${active}
