...
import { assertInstanceOf, Bound } from '@blockexpanse/global/utils';
...
export const getMindmapRender =
  ...
  (bound, edgelessService) => {
    ...
    const t =
      edgelessService.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    const root: MindMapNode = {
      children: [],
      text: t(I18nKeys.editor.edgeless.mindMap, 'Mind Map'),
    ...
    for (let i = 0; i < 3; i++) {
      ...
      root.children.push({
        children: [],
        text: t(I18nKeys.editor.edgeless.text, 'Text'),
