import type { MindmapStyle } from '@blockexpanse/affine-model';
import type { BlockStdScope } from '@blockexpanse/block-std';
import type { Bound } from '@blockexpanse/global/utils';
...
import { modelContext, stdContext } from '@blockexpanse/block-std';
import { ErrorCode } from '@blockexpanse/global/exceptions';
    ...
    return html`<div class="mindmap-item">
      ...
      <affine-tooltip tip-position="top" .offset=${12}>
        ${getTooltipWithShortcut(
          (this.std.getOptional(I18nProvider)?.t ?? identityI18nFn)(
            I18nKeys.editor.edgeless.mindmapTools.importSupport,
  ...
  private _onImportMindMap(bound: Bound) {
    ...
    this.onImportMindMap?.(bound)
      ...
      .catch(e => {
        ...
        toastI18n(
          this.edgeless.host,
          I18nKeys.editor.toast.importFailed,
      ...
      <div class="text-and-mindmap">
        <div class="text-item">
          ...
          <affine-tooltip tip-position="top" .offset=${12}>
            ${getTooltipWithShortcut(
              (this.std.getOptional(I18nProvider)?.t ?? identityI18nFn)(
                I18nKeys.editor.edgeless.edgelessText,
                'Edgeless Text'
              ),
              'T'
            )}
          ...
          return html`
            <div class="mindmap-item" data-is-active=${isActive}>
              ...
              <affine-tooltip tip-position="top" .offset=${12}>
                ${getTooltipWithShortcut(
                  (this.std.getOptional(I18nProvider)?.t ?? identityI18nFn)(
                    I18nKeys.editor.edgeless.mindMap,
                    'Mind Map'
                  ),
                  'M'
                )}
