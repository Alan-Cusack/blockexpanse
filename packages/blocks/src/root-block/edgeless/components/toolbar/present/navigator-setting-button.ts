import { NavigatorSettingsIcon } from '@blockexpanse/affine-components/icons';
import {
  EditPropsStore,
  I18nKeys,
  I18nProvider,
...
import { createButtonPopper } from '@blockexpanse/affine-shared/utils';
...
export class EdgelessNavigatorSettingButton extends WithDisposable(LitElement) {
  ...
  override render() {
    ...
    const settingsLabel = t(I18nKeys.editor.edgeless.settings, 'Settings');
    const playbackSettingsLabel = t(
      I18nKeys.editor.edgeless.playbackSettings,
      'Playback Settings'
    );
    const blackBackgroundLabel = t(
      I18nKeys.editor.edgeless.blackBackground,
      'Black background'
    );
    const hideToolbarLabel = t(
      I18nKeys.editor.edgeless.hideToolbar,
      'Hide toolbar'
    );
    const frameOrderLabel = t(
      I18nKeys.editor.edgeless.frameOrder,
      'Frame Order'
    );
    return html`
      <edgeless-tool-icon-button
        class="navigator-setting-button"
        .tooltip=${this.popperShow ? '' : settingsLabel}
        @click=${() => {
      ...
      >
        <div class="item-container header">
          <div class="text title">${playbackSettingsLabel}</div>
        </div>

        <div class="item-container">
          <div class="text">${blackBackgroundLabel}</div>
        ...
        <div class="item-container">
          <div class="text">${hideToolbarLabel}</div>
        ...
        ${this.includeFrameOrder
          ? html` <div class="divider"></div>
              <div class="item-container header">
                <div class="text title">${frameOrderLabel}</div>
              </div>
