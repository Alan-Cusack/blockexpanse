import { type ShapeName, ShapeType } from '@blockexpanse/affine-model';
import {
  I18nKeys,
  I18nProvider,
  identityI18nFn,
...
export class EdgelessShapeToolButton extends EdgelessToolbarToolMixin(
  ...
  override render() {
    ...
    const shapeLabel = t(I18nKeys.editor.edgeless.shape, 'Shape');

    return html`
      <edgeless-toolbar-button
        class="edgeless-shape-button"
        .tooltip=${this.popper ? '' : getTooltipWithShortcut(shapeLabel, 'S')}
        .tooltipOffset=${5}
        .active=${active}
      >
