import type { GfxController, GfxModel } from '@blockexpanse/block-std/gfx';
...
import { assertType, Bound } from '@blockexpanse/global/utils';
...
export class AutoCompleteTextOverlay extends AutoCompleteTargetOverlay {
  ...
  override render(ctx: CanvasRenderingContext2D, _rc: RoughCanvas) {
    ...
    const t = this.gfx.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    ctx.fillText(
      t(
        I18nKeys.editor.edgeless.autoComplete.typeSlashInsert,
        "Type '/' to insert"
      ),
      x + w / 2,
      y + h / 2
...
export class AutoCompleteNoteOverlay extends AutoCompleteTargetOverlay {
  ...
  override render(ctx: CanvasRenderingContext2D, _rc: RoughCanvas) {
    ...
    ctx.fillText(
      t(
        I18nKeys.editor.edgeless.autoComplete.typeSlashCommand,
        "Type '/' for command"
      ),
      x + 24,
      y + h / 2
