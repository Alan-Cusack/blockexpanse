...
import { on, once, stopPropagation } from '@blockexpanse/affine-shared/utils';
...
export class EdgelessColorPicker extends SignalWatcher(
  ...
  override render() {
    ...
    const tabs = TABS.map(tab => ({
      ...
      name:
        tab.type === 'colors'
          ? t(I18nKeys.editor.colorPicker.colors, 'Colors')
          : t(I18nKeys.editor.edgeless.custom, 'Custom'),
    }));
    return html`
      <header>
        <nav>
          ...
          ({ type, hsva }) => html`
            ...
            >
              ...
              >
                ...
                <div>
                  ${type === 'light'
                    ? t(I18nKeys.editor.colorPicker.light, 'Light')
                    : t(I18nKeys.editor.colorPicker.dark, 'Dark')}
                </div>
              </button>
            </div>
          `
