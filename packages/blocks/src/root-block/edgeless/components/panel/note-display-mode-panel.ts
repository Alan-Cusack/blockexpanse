import { EdgelessIcon, PageIcon } from '@blockexpanse/affine-components/icons';
...
export class NoteDisplayModePanel extends WithDisposable(LitElement) {
  ...
  private _DisplayModeLabel(mode: NoteDisplayMode) {
    ...
    switch (mode) {
      case NoteDisplayMode.DocAndEdgeless:
        return t(I18nKeys.editor.edgeless.noteTools.inBoth, 'In Both');
      case NoteDisplayMode.DocOnly:
        return t(I18nKeys.editor.edgeless.noteTools.inPageOnly, 'In Page Only');
      case NoteDisplayMode.EdgelessOnly:
        return t(
          I18nKeys.editor.edgeless.noteTools.inEdgelessOnly,
          'In Edgeless Only'
        );
    }
  }
