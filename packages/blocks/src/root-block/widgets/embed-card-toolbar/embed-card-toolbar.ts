...
import { type BlockStdScope, WidgetComponent } from '@blockexpanse/block-std';
import { type BlockModel, DocCollection } from '@blockexpanse/store';
...
> {
   ...
   */
  moreGroups: MenuItemGroup<EmbedCardToolbarContext>[] = cloneGroups(
    createBuiltInGroups()
  );
  ...
  private _openMenuButton() {
    ...
    const openLabel = t(I18nKeys.editor.action.open, 'Open');
  ...
  override render() {
    ...
    const t = this.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    const copyLinkLabel = t(I18nKeys.editor.action.copyLink, 'Copy link');
    const editLabel = t(I18nKeys.editor.action.edit, 'Edit');
