...
import { cloneGroups } from '@blockexpanse/affine-components/toolbar';
import {
  I18nProvider,
  identityI18nFn,
...
import { createMoreGroups, createPrimaryGroups } from './config.js';
...
> {
  ...
  moreGroups: MenuItemGroup<ImageToolbarContext>[] =
    cloneGroups(createMoreGroups());

  primaryGroups: MenuItemGroup<ImageToolbarContext>[] = cloneGroups(
    createPrimaryGroups()
  );
  ...
  override firstUpdated() {
    ...
    this.primaryGroups = cloneGroups(createPrimaryGroups(t));
    this.moreGroups = getMoreMenuConfig(this.std).configure(
      cloneGroups(createMoreGroups(t))
    );
