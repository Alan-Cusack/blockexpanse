...
import { createSlashMenuConfig } from './create-config.js';
...
export class AffineSlashMenuWidget extends WidgetComponent {
  ...
  private _configLocalized = false;
  ...
  override connectedCallback() {
    ...
    if (!this._configLocalized && this.config === defaultSlashMenuConfig) {
      const i18n = this.std.getOptional(I18nProvider);
      this.config = createSlashMenuConfig(i18n?.t ?? identityI18nFn);
      this._configLocalized = true;
    }
