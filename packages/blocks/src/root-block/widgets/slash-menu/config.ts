...
import { assertType } from '@blockexpanse/global/utils';
import {
...
import { Slice, Text } from '@blockexpanse/store';
...
export type SlashMenuConfig = {
  triggerKeys: string[];
  ignoreBlockTypes: BlockExpanse.Flavour[];
  items: SlashMenuItem[];
  maxHeight: number;
...
export const defaultSlashMenuConfig: SlashMenuConfig = {
  ...
  items: [
...

    {
      ...
      description: 'Create an inline equation.',
      icon: TeXIcon({
        width: '20',
        height: '20',
      }),
    ...
    { groupName: 'Style' },
    ...textFormatConfigs
      .filter(i => !['code', 'link'].includes(i.id))
