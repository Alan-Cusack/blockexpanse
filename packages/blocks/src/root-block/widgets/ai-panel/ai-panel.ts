import type { BaseSelection } from '@blockexpanse/block-std';
...
export class AffineAIPanelWidget extends WidgetComponent {
  static override styles = css`
    ...
    .input-action-list ai-item-list {
      --item-padding: 4px;
    }
  ...
  showDiscardModal = () => {
    ...
    return notification
      .confirm({
        title: t(I18nKeys.editor.ai.discardTitle, 'Discard the AI result'),
        message: t(
          I18nKeys.editor.ai.discardMessage,
          'Do you want to discard the results the AI just generated?'
        ),
        cancelText: t(I18nKeys.editor.common.cancel, 'Cancel'),
        confirmText: t(I18nKeys.editor.ai.discard, 'Discard'),
  ...
  override render() {
    ...
    const inputGroups = filterAIItemGroup(
      this.host,
      config.inputStateConfig?.groups ?? []
    );
