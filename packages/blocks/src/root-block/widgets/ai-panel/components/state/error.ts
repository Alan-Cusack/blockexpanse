...
export class AIPanelError extends WithDisposable(LitElement) {
  ...
  override render() {
    ...
    const answerLabel = t(I18nKeys.editor.ai.answer, 'Answer');
    const cancelLabel = t(I18nKeys.editor.common.cancel, 'Cancel');
    ...
    const errorTemplate = choose(
        ...
        [
          ...
          () =>
            html` <div class="error-info">
                ${t(
                  I18nKeys.editor.ai.unauthorized,
              ...
              <div class="action-button-group">
                ...
                <div @click=${this.config.login} class="action-button primary">
                  <span>${t(I18nKeys.editor.ai.login, 'login')}</span>
                </div>
              </div>`,
        ],
        [
          ...
          () =>
            html` <div class="error-info">
                ${t(
                  I18nKeys.editor.ai.paymentRequired,
              ...
              <div class="action-button-group">
                ...
                >
                  <span>${t(I18nKeys.editor.ai.upgrade, 'Upgrade')}</span>
                </div>
              </div>`,
        ],
      ],
      ...
      () => {
        ...
        const errorOccurred = t(
          I18nKeys.editor.ai.errorOccurred,
          'An error occurred'
        );
        const errorRetry = t(
          I18nKeys.editor.ai.errorRetry,
          'Please try again later. If this issue persists, please let us know at'
        );
        const error = tip
          ? html`<span class="error-tip"
              >${errorOccurred}<affine-tooltip
