import type { RichText } from '@blockexpanse/affine-components/rich-text';
...
export class AffineFormatBarWidget extends WidgetComponent {
  ...
  private _calculatePlacement() {
    ...
    this.disposables.add(
      this._selectionManager.slots.changed.on(() => {
        const update = async () => {
          ...
          // database / simple table cells: keep native format bar when text is selected
          if (
            this.host.selection.value.some(
              s => s.is('database') || s.is('table')
            )
          ) {
            const range = this.nativeRange;
    ...
    this.disposables.addFromEvent(document, 'selectionchange', () => {
      ...
      const tableSelection = this.host.selection.find('table');
      if (!databaseSelection && !tableSelection) {
        return;
      }
      ...
      if (databaseSelection) {
        ...
        // check database table view selection
        if (
        ...
        // simple table (`affine:table`): show when text is selected inside the block
        const tableBlock = this.host.view.getBlock(tableSelection.blockId);
        const range = this.nativeRange;
        if (
          !tableBlock ||
          !range ||
          range.collapsed ||
          !range.intersectsNode(tableBlock)
        ) {
          return reset();
        }
        this._displayType = 'native';
  ...
  /**
   * Opt-in Ask AI entry at the end of the format toolbar (before More).
   * Requires `affine-ai-panel-widget` to be mounted and configured by the host.
   */
  addAskAiButton() {
    const t = this.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    const askAiLabel = t(I18nKeys.editor.ai.askAi, 'Ask AI');
    return this.addDivider().addRawConfigItems([
      {
        type: 'custom',
        render: formatBar => html`
          <editor-icon-button
            class="ask-ai-button"
            data-testid="ask-ai"
            ...
            @click=${(e: MouseEvent) => {
              e.stopPropagation();
              const rootId = formatBar.host.doc.root?.id;
              const aiPanel = rootId
                ? formatBar.host.view.getWidget(AFFINE_AI_PANEL_WIDGET, rootId)
                : null;
              if (
                !aiPanel ||
                !('toggle' in aiPanel) ||
                typeof (aiPanel as { toggle: unknown }).toggle !== 'function'
              ) {
                return;
              }
              const block = formatBar.selectedBlocks[0];
              (aiPanel as { toggle: (el: Element) => void }).toggle(
                block ?? formatBar
              );
            }}
          >
            ${AIStarIcon}<span class="label medium">${askAiLabel}</span>
          </editor-icon-button>
        `,
      },
    ]);
  ...
  addTextStyleToggle(config: {
    ...
    const t = this.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    const name =
      createTextFormatConfigs(t).find(item => item.id === key)?.name ??
