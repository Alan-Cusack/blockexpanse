...
import { assertEquals } from '@blockexpanse/global/utils';
...
export class PieNodeContent extends LitElement {
  ...
  private _renderCenterNodeContent() {
    ...
    const { label } = model;
    const translatedLabel = this._translateLabel(label);
    const centerLabelOrIcon = this.node.icon ?? translatedLabel;

    return isActiveNode
      ? hoveredNode
        ? this._translateLabel(hoveredNode.model.label)
        : centerLabelOrIcon
      : centerLabelOrIcon;
  }
  ...
  private _t() {
    return (
      this.node.menu.rootComponent.std.getOptional(I18nProvider)?.t ??
      identityI18nFn
    );
  }

  private _translateLabel(label: string) {
    return translatePieLabel(this._t(), label);
  }
