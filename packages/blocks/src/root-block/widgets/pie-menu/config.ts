...
import { isControlledKeyboardEvent } from '@blockexpanse/affine-shared/utils';
...
pie.command({
  ...
  icon: ({ rootComponent }) => {
    ...
    if (type === 'frameNavigator') {
      const t =
        rootComponent.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
      const stopLabel = t(I18nKeys.editor.edgeless.pie.stop, 'Stop');
      return html`
        <span
          style="${styleMap({
            color: '#eb4335',
