...
export class LanguageListButton extends WithDisposable(
  ...
  private _clickLangBtn = () => {
    ...
    const t =
      this.blockComponent.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    const options: FilterableListOptions = {
      placeholder: t(
        I18nKeys.editor.code.searchLanguage,
  ...
  override connectedCallback(): void {
    ...
    const langList = localStorage.getItem('blockexpanse:code-block:lang-list');
    if (langList) {
    ...
    this.disposables.add(() => {
      localStorage.setItem(
        'blockexpanse:code-block:lang-list',
