import { updateXYWH } from '@blockexpanse/affine-block-surface';
import {
  AlignBottomIcon,
...
import { AutoTidyUpIcon, ResizeTidyUpIcon } from '@blockexpanse/icons/lit';
...
function getAlignmentLabel(
  ...
  switch (name) {
    case Alignment.Left:
      return t(I18nKeys.editor.edgeless.align.left, name);
    case Alignment.Horizontally:
      return t(I18nKeys.editor.edgeless.align.horizontally, name);
    case Alignment.Right:
      return t(I18nKeys.editor.edgeless.align.right, name);
    case Alignment.DistributeHorizontally:
      return t(I18nKeys.editor.edgeless.align.distributeHorizontally, name);
    case Alignment.Top:
      return t(I18nKeys.editor.edgeless.align.top, name);
    case Alignment.Vertically:
      return t(I18nKeys.editor.edgeless.align.vertically, name);
    case Alignment.Bottom:
      return t(I18nKeys.editor.edgeless.align.bottom, name);
    case Alignment.DistributeVertically:
      return t(I18nKeys.editor.edgeless.align.distributeVertically, name);
    case Alignment.AutoArrange:
      return t(I18nKeys.editor.edgeless.align.autoArrange, name);
    case Alignment.AutoResize:
      return t(I18nKeys.editor.edgeless.align.autoResize, name);
    default:
      return name;
  }
}
...
export class EdgelessAlignButton extends WithDisposable(LitElement) {
  ...
  private _updateXYWH(ele: BlockExpanse.EdgelessModel, bound: Bound) {
  ...
  private renderIcons(icons: AlignmentIcon[]) {
    ...
    return html`
      ${repeat(
        ...
        ({ name, content }) => {
          const label = getAlignmentLabel(name, t);
          return html`
            <editor-icon-button
              aria-label=${label}
              .tooltip=${label}
  ...
  override render() {
    const t = this.edgeless.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    const alignObjectsLabel = t(
      I18nKeys.editor.edgeless.align.objects,
      'Align objects'
    );
    ...
    return html`
      <editor-menu-button
        .button=${html`
          <editor-icon-button
            aria-label=${alignObjectsLabel}
            .tooltip=${alignObjectsLabel}
          >
