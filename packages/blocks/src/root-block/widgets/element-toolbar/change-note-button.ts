...
function getDisplayModeLabel(
  ...
  switch (mode) {
    case NoteDisplayMode.DocAndEdgeless:
      return t(I18nKeys.editor.edgeless.noteTools.both, 'Both');
    case NoteDisplayMode.EdgelessOnly:
      return t(I18nKeys.editor.edgeless.noteTools.edgelessOnly, 'Edgeless');
    case NoteDisplayMode.DocOnly:
      return t(I18nKeys.editor.edgeless.noteTools.pageOnly, 'Page');
  }
}

function getTranslatedSizeList(
  t: (key: string, fallback: string) => string
): { name: string; value: number }[] {
  return SIZE_LIST.map(({ name, value }) => {
    switch (name) {
      case 'None':
        return { name: t(I18nKeys.editor.edgeless.size.none, name), value };
      case 'Small':
        return { name: t(I18nKeys.editor.edgeless.size.small, name), value };
      case 'Medium':
        return { name: t(I18nKeys.editor.edgeless.size.medium, name), value };
      case 'Large':
        return { name: t(I18nKeys.editor.edgeless.size.large, name), value };
      case 'Huge':
        return { name: t(I18nKeys.editor.edgeless.size.huge, name), value };
      default:
        return { name, value };
    }
  });
...
export class EdgelessChangeNoteButton extends WithDisposable(LitElement) {
  ...
  override render() {
    ...
    const displayModeLabel = t(
      I18nKeys.editor.edgeless.noteTools.displayMode,
      'Display mode'
    );
    ...
    const shadowStyleLabel = t(
      I18nKeys.editor.edgeless.noteTools.shadowStyle,
      'Shadow style'
    );
    const borderStyleLabel = t(
      I18nKeys.editor.edgeless.noteTools.borderStyle,
      'Border style'
    );
    const cornersLabel = t(
      I18nKeys.editor.edgeless.noteTools.corners,
      'Corners'
    );
    const cuttingModeLabel = t(
      I18nKeys.editor.edgeless.noteTools.cuttingMode,
      'Cutting mode'
    );
    const autoHeightLabel = t(
      I18nKeys.editor.edgeless.noteTools.autoHeight,
      'Auto height'
    );
    const customizedHeightLabel = t(
      I18nKeys.editor.edgeless.noteTools.customizedHeight,
      'Customized height'
    );
    const scaleLabel = t(I18nKeys.editor.edgeless.scale, 'Scale');
    const translatedSizeList = getTranslatedSizeList(t);
    ...
    const currentMode = getDisplayModeLabel(displayMode, t);
    const showInLabel = t(I18nKeys.editor.edgeless.noteTools.showIn, 'Show in');
    const onlyOne = len === 1;
    ...
    const buttons = [
      onlyOne && this._advancedVisibilityEnabled
        ? html`
            <span class="display-mode-button-label">${showInLabel}</span>
            <editor-menu-button
              .contentPadding=${'8px'}
              .button=${html`
                <editor-icon-button
                  aria-label=${displayModeLabel}
                  .tooltip=${displayModeLabel}
      ...
      isDocOnly
        ...
        : html`
            <editor-menu-button
              ...
              .button=${html`
                <editor-icon-button
                  aria-label=${shadowStyleLabel}
                  .tooltip=${shadowStyleLabel}
                >
            ...
            <editor-menu-button
              .button=${html`
                <editor-icon-button
                  aria-label=${borderStyleLabel}
                  .tooltip=${borderStyleLabel}
                >
            ...
            <editor-menu-button
              ...
              .button=${html`
                <editor-icon-button
                  aria-label=${cornersLabel}
                  .tooltip=${cornersLabel}
                >
      ...
      onlyOne && this._advancedVisibilityEnabled
        ? html`
            <editor-icon-button
              aria-label=${cuttingModeLabel}
              .tooltip=${getTooltipWithShortcut(cuttingModeLabel, '-')}
      ...
      html`
        <editor-icon-button
          aria-label=${collapse ? autoHeightLabel : customizedHeightLabel}
          .tooltip=${collapse ? autoHeightLabel : customizedHeightLabel}
