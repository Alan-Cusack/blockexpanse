...
import type { GfxModel } from '@blockexpanse/block-std/gfx';
...
import { LockIcon, UnlockIcon } from '@blockexpanse/icons/lit';
...
export class EdgelessLockButton extends SignalWatcher(
  ...
  override render() {
    ...
    >
      ...
      ${hasLocked
        ? html`<span class="label medium"
            >${t(
              I18nKeys.editor.edgeless.clickToUnlock,
              'Click to unlock'
            )}</span
          >`
        : nothing}
