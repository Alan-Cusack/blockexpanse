...
import { getDocContentWithMaxLength } from '@blockexpanse/affine-block-embed';
import {
  CaptionIcon,
  CenterPeekIcon,
  CopyIcon,
...
import { notifyLinkedDocSwitchedToEmbed } from '@blockexpanse/affine-components/notification';
...
import { type AliasInfo, BookmarkStyles } from '@blockexpanse/affine-model';
import {
  EmbedOptionProvider,
  type EmbedOptions,
...
export class EdgelessChangeEmbedCardButton extends WithDisposable(LitElement) {
  ...
  private get _getCardStyleOptions(): {
    ...
    return [
      ...
      },
      {
        ...
        tooltip: t(
          I18nKeys.editor.edgeless.largeVerticalStyle,
      ...
      },
      {
        ...
        tooltip: t(
          I18nKeys.editor.edgeless.smallVerticalStyle,
  ...
  get _originalDocTitle() {
    ...
    const t = this.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    return doc?.meta?.title || t(I18nKeys.editor.common.untitled, 'Untitled');
  }
  ...
  private _openMenuButton() {
    ...
    return html`
      <editor-menu-button
        ...
        .button=${html`
          <editor-icon-button
            aria-label=${t(I18nKeys.editor.action.open, 'Open')}
  ...
  private _viewSelector() {
    if (this._canConvertToEmbedView || this._isEmbedView) {
      ...
      const buttons = [
        {
          type: 'card',
          label: t(I18nKeys.editor.action.cardView, 'Card view'),
          action: () => this._convertToCardView(),
          disabled: this.model.doc.readonly,
        },
        {
          type: 'embed',
          label: t(I18nKeys.editor.action.embedView, 'Embed view'),
      ...
      return html`
        <editor-menu-button
          ...
          .button=${html`
            <editor-icon-button
              aria-label=${t(I18nKeys.editor.action.switchView, 'Switch view')}
              .justify=${'space-between'}
              .labelHeight=${'20px'}
  ...
  override render() {
    ...
    const buttons = [
      ...
      isHtmlBlockModel
        ...
        : html`
            <editor-icon-button
              aria-label=${t(I18nKeys.editor.edgeless.clickLink, 'Click link')}
              .tooltip=${t(I18nKeys.editor.edgeless.clickLink, 'Click link')}
            ...
            <editor-icon-button
              aria-label=${t(I18nKeys.editor.action.edit, 'Edit')}
              .tooltip=${t(I18nKeys.editor.action.edit, 'Edit')}
        ...
        ? html`
            <editor-menu-button
              ...
              .button=${html`
                <editor-icon-button
                  aria-label=${t(
                    I18nKeys.editor.action.cardStyle,
                    'Card style'
                  )}
                  .tooltip=${t(I18nKeys.editor.action.cardStyle, 'Card style')}
                >
                  ${PaletteIcon}
                </editor-icon-button>
              `}
      ...
      'caption' in model
        ? html`
            <editor-icon-button
              aria-label=${t(I18nKeys.editor.action.addCaption, 'Add caption')}
              .tooltip=${t(I18nKeys.editor.action.addCaption, 'Add caption')}
      ...
      isHtmlBlockModel
        ...
        : html`
            <editor-menu-button
              ...
              .button=${html`
                <editor-icon-button
                  aria-label=${t(I18nKeys.editor.edgeless.scale, 'Scale')}
                  .tooltip=${t(I18nKeys.editor.edgeless.scale, 'Scale')}
                  .justify=${'space-between'}
