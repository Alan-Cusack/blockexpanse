...
import { LayoutType, MindmapStyle } from '@blockexpanse/affine-model';
import {
  EditPropsStore,
  I18nKeys,
  I18nProvider,
...
function getLayoutItemLabel(
  name: LayoutItem['name'],
  t: (key: string, fallback: string) => string
): string {
  switch (name) {
    case 'Left':
      return t(I18nKeys.editor.edgeless.mindmapTools.left, name);
    case 'Radial':
      return t(I18nKeys.editor.edgeless.mindmapTools.radial, name);
    case 'Right':
      return t(I18nKeys.editor.edgeless.mindmapTools.right, name);
    default:
      return name;
  }
}
...
export class EdgelessChangeMindmapLayoutPanel extends LitElement {
  ...
  override render() {
    const t = this.t ?? identityI18nFn;
    return repeat(
      MINDMAP_LAYOUT_LIST,
      item => item.value,
      ({ name, value, icon }) => {
        const label = getLayoutItemLabel(name, t);
        return html`
          <editor-icon-button
            aria-label=${label}
            .tooltip=${label}
  ...
  @property({ attribute: false })
  accessor t: ((key: string, fallback: string) => string) | undefined =
    undefined;
}
...
export class EdgelessChangeMindmapButton extends WithDisposable(LitElement) {
  ...
  override render() {
    ...
    const layoutLabel = t(I18nKeys.editor.edgeless.layout, 'Layout');
