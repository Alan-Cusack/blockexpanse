import { WebIcon16 } from '@blockexpanse/affine-components/icons';
import {
  I18nKeys,
  I18nProvider,
  identityI18nFn,
...
import { getHostName } from '@blockexpanse/affine-shared/utils';
import { ShadowlessElement } from '@blockexpanse/block-std';
import { WithDisposable } from '@blockexpanse/global/utils';
import { OpenInNewIcon } from '@blockexpanse/icons/lit';
...
export class BookmarkCard extends WithDisposable(ShadowlessElement) {
  ...
  override render() {
    ...
    const t = this.bookmark.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
    const titleText = this.loading
      ? t(I18nKeys.editor.common.loading, 'Loading...')
      : !title
        ? this.error
          ? (domainName ?? t(I18nKeys.editor.embed.linkCard, 'Link card'))
          : ''
        : title;
    ...
    const descriptionText = this.loading
      ...
      : !description
        ? this.error
          ? t(
              I18nKeys.editor.embed.linkFailed,
