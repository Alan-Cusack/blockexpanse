import type { NoteBlockModel } from '@blockexpanse/affine-model';

import { NoteBlockSchema } from '@blockexpanse/affine-model';
import { DragHandleConfigExtension } from '@blockexpanse/affine-shared/services';
...
export class NoteBlockService extends BlockService {
  ...
  private _onBlockShiftDown = (cmd: BlockExpanse.CommandChain) => {
    return cmd
      .getBlockSelections()
  ...
  private _onBlockShiftUp = (cmd: BlockExpanse.CommandChain) => {
    return cmd
      .getBlockSelections()
      .inline<'currentSelectionPath' | 'anchorBlock'>((ctx, next) => {
        const blockSelections = ctx.currentBlockSelections;
