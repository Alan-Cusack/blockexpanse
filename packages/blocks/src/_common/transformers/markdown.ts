import type { Doc, DocCollection } from '@blockexpanse/store';

import { BlockExpanseError, ErrorCode } from '@blockexpanse/global/exceptions';
import { assertExists, sha } from '@blockexpanse/global/utils';
import { extMimeMap, Job } from '@blockexpanse/store';
...
async function exportDoc(doc: Doc) {
  ...
  if (markdownResult.assetsIds.length > 0) {
    if (!job.assets) {
      throw new BlockExpanseError(ErrorCode.ValueNotExists, 'No assets found');
    }
