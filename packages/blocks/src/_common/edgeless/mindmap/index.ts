import type { Viewport } from '@blockexpanse/block-std/gfx';

import { MindmapElementModel } from '@blockexpanse/affine-model';

export function isMindmapNode(el: BlockExpanse.EdgelessModel) {
  return (
    el.group instanceof MindmapElementModel || el instanceof MindmapElementModel
  );
}

export function isSingleMindMapNode(els: BlockExpanse.EdgelessModel[]) {
