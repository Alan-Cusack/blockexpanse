import type { AliasInfo } from '@blockexpanse/affine-model';
import type {
  BlockComponent,
  BlockStdScope,
  EditorHost,
...
} from '@blockexpanse/affine-components/notification';
...
import { FONT_SM, FONT_XS } from '@blockexpanse/affine-shared/styles';
...
export class EmbedCardEditModal extends SignalWatcher(
  ...
  private _onSave = () => {
    ...
    if (title.length === 0) {
      toastI18n(
        this.host,
        I18nKeys.editor.toast.emptyTitle,
            ...
            [
              ...
              () => html`
                <button
                  ...
                  @click=${this._onReset}
                >
                  ${t(I18nKeys.editor.common.reset, 'Reset')}
                </button>
              `,
            ],
            [
              ...
              () => html`
                <button
                  ...
                  @click=${this._hide}
                >
                  ${t(I18nKeys.editor.common.cancel, 'Cancel')}
                </button>
              `,
            ],
          ])}
