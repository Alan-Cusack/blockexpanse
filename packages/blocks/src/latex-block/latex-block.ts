import type { LatexBlockModel } from '@blockexpanse/affine-model';
        ...
        if (latex.length === 0) {
          ...
          render(
            html`<span class="latex-block-empty-placeholder"
              >${t(I18nKeys.editor.latex.equation, 'Equation')}</span
            >`,
            katexContainer
          );
        } else {
          try {
            ...
            render(
              html`<span class="latex-block-error-placeholder"
                >${t(I18nKeys.editor.latex.error, 'Error equation')}</span
              >`,
              katexContainer
            );
          }
