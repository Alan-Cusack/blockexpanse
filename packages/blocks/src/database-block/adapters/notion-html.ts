import { DatabaseBlockSchema } from '@blockexpanse/affine-model';
import {
...
import { getTagColor } from '@blockexpanse/data-view';
import { type BlockSnapshot, nanoid } from '@blockexpanse/store';
...
type BlockexpanseTableRow = Record<
  string,
  {
    columnId: string;
    value: unknown;
        ...
        switch (o.node.tagName) {
          case 'th': {
            ...
            const columnType = Array.isArray(columnTypeClass)
              ...
              : 'rich-text';
            walkerContext.pushGlobalContextStack<BlockexpanseTableColumn>(
              'hast:table:column',
              {
                type: columnType,
          ...
          case 'tr': {
            if (
              ...
              const columns =
                walkerContext.getGlobalContextStack<BlockexpanseTableColumn>(
                  'hast:table:column'
                );
      ...
      leave: (o, context) => {
        ...
        switch (o.node.tagName) {
          case 'table': {
            ...
            walkerContext
              .getGlobalContextStack<BlockexpanseTableRow>('hast:table:rows')
              .map((row, i) => {
