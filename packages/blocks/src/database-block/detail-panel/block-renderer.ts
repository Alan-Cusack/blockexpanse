...
import type { DetailSlotProps } from '@blockexpanse/data-view';
import type {
  KanbanSingleView,
  TableSingleView,
} from '@blockexpanse/data-view/view-presets';

import { DefaultInlineManagerExtension } from '@blockexpanse/affine-components/rich-text';
import {
  I18nKeys,
  I18nProvider,
  identityI18nFn,
  ...
  implements DetailSlotProps
{
  ...
  protected override render(): unknown {
    ...
    const untitled = (
      this.host.std.getOptional(I18nProvider)?.t ?? identityI18nFn
    )(I18nKeys.editor.dataView.untitled, 'Untitled');
    return html`
      ${this.renderIcon()}
      <rich-text
        .yText=${model.text}
