import type { DatabaseBlockModel } from '@blockexpanse/affine-model';
...
import { DragIndicator } from '@blockexpanse/affine-components/drag-indicator';
import { PeekViewProvider } from '@blockexpanse/affine-components/peek';
import { toast, toastI18n } from '@blockexpanse/affine-components/toast';
import { NOTE_SELECTOR } from '@blockexpanse/affine-shared/consts';
import {
  DocModeProvider,
  I18nKeys,
  I18nProvider,
...
import { RANGE_SYNC_EXCLUDE_ATTR } from '@blockexpanse/block-std';
import {
  createRecordDetail,
...
import { widgetPresets } from '@blockexpanse/data-view/widget-presets';
import { Rect } from '@blockexpanse/global/utils';
import {
  CopyIcon,
  DeleteIcon,
  MoreHorizontalIcon,
...
> {
    ...
    const options = this.optionsConfig.configure(this.model, {
      items: [
        menu.input({
          ...
          placeholder: t(I18nKeys.editor.dataView.untitled, 'Untitled'),
          onChange: text => {
        ...
        menu.action({
          prefix: CopyIcon(),
          name: t(I18nKeys.editor.dataView.copy, 'Copy'),
          select: () => {
        ...
        menu.group({
          items: [
            menu.action({
              ...
              name: t(
                I18nKeys.editor.dataView.deleteDatabase,
                'Delete Database'
              ),
              select: () => {
  ...
  private get _t() {
    return this.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
  }
