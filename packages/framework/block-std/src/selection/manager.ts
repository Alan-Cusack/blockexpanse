...
import { DisposableGroup, Slot } from '@blockexpanse/global/utils';
import { nanoid, type StackItem } from '@blockexpanse/store';
...
export class SelectionManager extends LifeCycleWatcher {
  ...
  create<T extends BlockExpanse.SelectionType>(
    type: T,
    ...args: ConstructorParameters<BlockExpanse.Selection[T]>
  ): BlockExpanse.SelectionInstance[T] {
    ...
    return new ctor(...args) as BlockExpanse.SelectionInstance[T];
  }
  ...
  filter<T extends BlockExpanse.SelectionType>(type: T) {
    return this.filter$(type).value;
  }

  filter$<T extends BlockExpanse.SelectionType>(type: T) {
    return computed(() =>
      this.value.filter((sel): sel is BlockExpanse.SelectionInstance[T] =>
        sel.is(type)
      )
    );
  }

  find<T extends BlockExpanse.SelectionType>(type: T) {
    return this.find$(type).value;
  }

  find$<T extends BlockExpanse.SelectionType>(type: T) {
    return computed(() =>
      this.value.find((sel): sel is BlockExpanse.SelectionInstance[T] =>
        sel.is(type)
      )
    );
  }
