 ...
 * ```
 *
 * You should always add the command to the global interface `BlockExpanse.Commands`
 * ```ts
 * declare global {
...
export class CommandManager extends LifeCycleWatcher {
  ...
  private _createChain = (
    methods: Record<BlockExpanse.CommandName, unknown>,
    _cmds: Command[]
  ): Chain => {
    ...
    return {
      ...
      run: function (this: Chain) {
        ...
        try {
          ...
          ctx = runCmds(ctx as BlockExpanse.CommandContext, [
            ...cmds,
            (_, next) => {
              success = true;
              next();
   ...
   * @param name
   * @param command
   * Make sure to also add the command to the global interface `BlockExpanse.Commands`
   * ```ts
   ...
   * ```
   */
  add<N extends BlockExpanse.CommandName>(
    name: N,
    command: BlockExpanse.Commands[N]
  ): CommandManager;
  ...
  override created() {
    ...
    this.std.provider.getAll(CommandIdentifier).forEach((command, key) => {
      add(key as keyof BlockExpanse.Commands, command);
    });
  }
   ...
   */
  exec<K extends keyof BlockExpanse.Commands>(
    command: K,
    ...payloads: IfAllKeysOptional<
      Omit<InDataOfCommand<BlockExpanse.Commands[K]>, keyof InitCommandCtx>,
      [
        inData: void | Omit<
          InDataOfCommand<BlockExpanse.Commands[K]>,
...
function runCmds(ctx: BlockExpanse.CommandContext, [cmd, ...rest]: Command[]) {
  let _ctx = ctx;
  if (cmd) {
    cmd(ctx, data => {
