...
export const DEFAULT_DB_NAME = 'blockexpanse-local';
...
interface BlockExpanseBinaryDB extends DBSchema {
  collection: {
    key: string;
...
export function upgradeDB(db: IDBPDatabase<BlockExpanseBinaryDB>) {
...
export class IndexedDBDocSource implements DocSource {
  ...
  dbPromise: Promise<IDBPDatabase<BlockExpanseBinaryDB>> | null = null;
  ...
  getDb() {
    if (this.dbPromise === null) {
      this.dbPromise = openDB<BlockExpanseBinaryDB>(this.dbName, dbVersion, {
        upgrade: upgradeDB,
      });
    }
