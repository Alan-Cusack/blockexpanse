...
import { type Logger, NoopLogger, Slot } from '@blockexpanse/global/utils';
import {
  AwarenessEngine,
...
export type DocCollectionOptions = {
  ...
  defaultFlags?: Partial<BlockExpanseFlags>;
  logger?: Logger;
  docSources?: {
    main: DocSource;
    shadows?: DocSource[];
...
export class DocCollection extends DocCollectionAddonType {
  ...
  constructor({
    ...
    this.id = id || '';
    this.doc = new BlockExpanseDoc({ guid: id });
