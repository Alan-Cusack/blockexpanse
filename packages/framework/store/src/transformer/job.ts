...
import { nextTick, Slot } from '@blockexpanse/global/utils';
...
export class Job {
  ...
  private _getCollectionMeta() {
    ...
    const { docs } = meta;
    if (!docs) {
      throw new BlockExpanseError(ErrorCode.TransformerError, 'Docs not found');
    }
    return {
  ...
  private async _insertBlockTree(
    ...
    for (let index = 0; index < nodes.length; index++) {
      ...
      doc.addBlock(
        flavour as BlockExpanse.Flavour,
        draft,
        parentId,
        actualIndex
      );
