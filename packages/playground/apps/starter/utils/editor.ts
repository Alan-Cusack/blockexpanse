...
import { AffineEditorContainer, CommentPanel } from '@blockexpanse/presets';

import {
  buildDemoAIPanelConfig,
  buildPageAIActionGroups,
} from '../../_common/ai/demo-config.js';
...
function configureFormatBar(formatBar: AffineFormatBarWidget) {
  // Shell default toolbar + opt-in Ask AI entry.
  toolbarDefaultConfig(formatBar).addAskAiButton();
}

function configureAIPanel(panel: AffineAIPanelWidget) {
  const t = panel.host.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
  // Page: action groups live in the AI panel input menu.
  // Edgeless: groups are shown by EdgelessCopilotPanel instead.
  const actionGroups = isInsidePageEditor(panel.host)
    ? buildPageAIActionGroups(t)
    : [];
  panel.config = buildDemoAIPanelConfig(panel, { t, actionGroups });
}

function configureEdgelessCopilot(widget: EdgelessCopilotWidget) {
  const t = widget.host.std.getOptional(I18nProvider)?.t ?? identityI18nFn;
  widget.groups = buildPageAIActionGroups(t);
}

function configureElementToolbar(toolbar: EdgelessElementToolbarWidget) {
  toolbar.addAskAiButton();
}
...
export async function mountDefaultDocEditor(collection: DocCollection) {
  ...
  class PatchPageServiceWatcher extends BlockServiceWatcher {
    ...
    override mounted() {
      ...
      const onWidgetConnected = pageRootService.specSlots.widgetConnected.on(
        view => {
          if (view.component instanceof AffineFormatBarWidget) {
            configureFormatBar(view.component);
          }
          if (view.component instanceof AffineAIPanelWidget) {
            configureAIPanel(view.component);
          }
          if (view.component instanceof EdgelessCopilotWidget) {
            configureEdgelessCopilot(view.component);
          }
          if (view.component instanceof EdgelessElementToolbarWidget) {
            configureElementToolbar(view.component);
          }
        }
      );
      pageRootService.disposables.add(onWidgetConnected);
    }
  }
