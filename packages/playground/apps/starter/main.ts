...
import * as blocks from '@blockexpanse/blocks';
import {
...
import * as globalUtils from '@blockexpanse/global/utils';
import * as editor from '@blockexpanse/presets';
import { effects as presetsEffects } from '@blockexpanse/presets/effects';
import * as store from '@blockexpanse/store';
...
async function main() {
  ...
  if (isE2E) {
    Object.defineProperty(window, '$blockexpanse', {
      value: Object.freeze({
        store,
        blocks,
    ...
    // test if blockexpanse can run in a web worker, SEE: tests/worker.spec.ts
