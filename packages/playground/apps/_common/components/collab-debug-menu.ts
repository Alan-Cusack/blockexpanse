...
import { EdgelessRootService, printToPdf } from '@blockexpanse/blocks';
import { type SerializedXYWH, SignalWatcher } from '@blockexpanse/global/utils';
...
import { type DocCollection, Text } from '@blockexpanse/store';
...
export class CollabDebugMenu extends SignalWatcher(ShadowlessElement) {
  ...
  private _setThemeMode(dark: boolean) {
    ...
    this._dark = dark;
    localStorage.setItem('blockexpanse:dark', dark ? 'true' : 'false');
    if (!html) return;
          ...
          <div class="top-container">
            <sl-dropdown placement="bottom" hoist>
              ...
              <sl-menu>
                ...
                <a
                  target="_blank"
                  href="https://github.com/Alan-Cusack/blockexpanse"
                >
                  <sl-menu-item>
  ...
  @state()
  private accessor _dark = localStorage.getItem('blockexpanse:dark') === 'true';
