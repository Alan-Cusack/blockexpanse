/**
 * @file Vendored from @toeverything/theme v1.1.1 (v1 entrypoint).
 *
 * Reconstructed from the bundled `dist/index.js` so that runtime values and
 * the public type surface match the upstream package exactly.
 */

import { grabbingCursor, grabCursor } from './cursors.js';
import { darkCssVariablesV2, lightCssVariablesV2 } from './v2/index.js';
 ...
 *
 * Mirrors the upstream `camelToKebab` helper: dashes become underscores
 * first, camel boundaries and trailing digits get a dash separator, then
 * whitespace/underscores collapse to dashes.
 */
export const camelToKebab = (input: unknown): string =>
  typeof input !== 'string'
    ? ''
    : input
        .replace(/-/g, '_')
...
/** Compose a CSS `cursor` shorthand from a data-URI and fallback keywords. */
const cursorCss = (dataUri: string, fallback: string[]): string =>
  `url(${dataUri}), ${fallback.join(', ')}`;

/** Map a theme key to its `--affine-<kebab>` CSS custom property name. */
export const themeToVar = <T extends string>(
  themeName: T
): `--affine-${string}` => `--affine-${camelToKebab(themeName)}`;

const toEntries = <T extends Record<string, unknown>>(
  value: T
): [string, unknown][] => Object.entries(value);
...
const APPLE_FONT_STACK =
  "apple-system, BlinkMacSystemFont, 'Helvetica Neue', Tahoma, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji','Segoe UI Symbol', 'Noto Color Emoji'";
const PRINT_FONT_STACK =
  "'Helvetica Neue', Tahoma, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji','Segoe UI Symbol', 'Noto Color Emoji'";

// ---------------------------------------------------------------------------
// baseTheme — static tokens unaffected by `data-theme`
// ---------------------------------------------------------------------------

export const baseTheme = {
  // font
  fontFamily: `'Inter', 'Source Sans 3', Poppins, ${APPLE_FONT_STACK}`,
  fontSerifFamily: `'Source Serif 4', 'Noto Serif' , serif, 'Cambria', ${APPLE_FONT_STACK}`,
  fontMonoFamily: `'Source Code Pro', 'IBM Plex Mono', 'Roboto Mono','Noto Sans Mono', 'Noto Sans CJK',  ${APPLE_FONT_STACK}`,
  fontSansFamily: `'Inter', 'Source Sans 3', Poppins, ${APPLE_FONT_STACK}`,
  fontNumberFamily: `'Roboto Mono', 'Noto Sans Mono', ${APPLE_FONT_STACK}`,
  fontCodeFamily: `'IBM Plex Mono', 'Space Mono', Consolas, Menlo, Monaco, Courier, monospace, ${APPLE_FONT_STACK}`,
  fontTitle: '36px',
  fontH1: '28px',
  fontH2: '26px',
  fontH3: '24px',
  ...
  // cursors
  grabbingCursor: cursorCss(grabbingCursor, ['grabbing']),
  grabCursor: cursorCss(grabCursor, ['grab']),
};

// ---------------------------------------------------------------------------
// lightTheme — baseTheme + light-mode color tokens
// ---------------------------------------------------------------------------
...
export const lightTheme = { ...baseTheme, ...lightThemeColors };

// ---------------------------------------------------------------------------
// darkTheme — baseTheme + dark-mode color tokens
// ---------------------------------------------------------------------------
...
export const darkTheme = { ...baseTheme, ...darkThemeColors };

// ---------------------------------------------------------------------------
// printTheme — font stack swaps only (for print output)
// ---------------------------------------------------------------------------

export const printTheme = {
  fontFamily: `'Inter', 'Source Sans 3', Poppins, ${PRINT_FONT_STACK}`,
  fontSerifFamily: `'Source Serif 4', 'Noto Serif' , serif, 'Cambria', ${PRINT_FONT_STACK}`,
  fontMonoFamily: `'Source Code Pro', 'IBM Plex Mono', 'Roboto Mono','Noto Sans Mono', 'Noto Sans CJK',  ${PRINT_FONT_STACK}`,
  fontSansFamily: `'Inter', 'Source Sans 3', Poppins, ${PRINT_FONT_STACK}`,
  fontNumberFamily: `'Roboto Mono', 'Noto Sans Mono', ${PRINT_FONT_STACK}`,
  fontCodeFamily: `'IBM Plex Mono', 'Space Mono', Consolas, Menlo, Monaco, Courier, monospace, ${PRINT_FONT_STACK}`,
};
...
export type AffineCssVariables = Record<
  `--affine-${Kebab<keyof AffineTheme>}`,
  string
>;
...
/**
 * Convert a theme object into a `{ ['--affine-<kebab>']: value }` record.
 * The value is the raw token (not a `var()` reference) — matching the
 * upstream bundled `g` helper.
 */
const toCssVariables = <T extends Record<string, string>>(
  theme: T
): Record<string, string> =>
  toEntries(theme).reduce<Record<string, string>>(
    (acc, [key, value]) => ({ ...acc, [themeToVar(key)]: value as string }),
    {}
  );

export const lightCssVariables: AffineCssVariables = toCssVariables(
  lightTheme
) as AffineCssVariables;
export const darkCssVariables: AffineCssVariables = toCssVariables(
  darkTheme
) as AffineCssVariables;
export const printCssVariables: AffineCssVariables = toCssVariables(
  printTheme
) as AffineCssVariables;

/** light-mode vars + v2 light vars (kebab-spaced keys). */
export const combinedLightCssVariables = {
  ...lightCssVariables,
  ...lightCssVariablesV2,
};

/** dark-mode vars + v2 dark vars. */
...
/**
 * Scoped light/dark maps: the **color-only** part (no base fonts/palettes)
 * merged with the v2 vars. Mirrors the upstream spread structure where the
 * scoped base is `toCssVariables(<colors only>)` plus the v2 maps.
 */
...
export function cssVar(
  ...
  const name = (key as string).startsWith('--')
    ? key
    : themeToVar(key as string);
