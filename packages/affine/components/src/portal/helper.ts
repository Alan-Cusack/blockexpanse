...
/**
 * Similar to `<blockexpanse-portal>`, but only renders once when called.
 *
...
export function createSimplePortal({
  ...
  if (identifyWrapper) {
    portalRoot.classList.add('blockexpanse-portal');
  }
  if (shadowDom) {
  ...
  // in order to avoid the issue that the floating element in blockexpanse cannot be clicked
