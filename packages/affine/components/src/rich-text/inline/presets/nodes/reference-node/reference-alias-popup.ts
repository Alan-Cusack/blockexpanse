...
import type { DeltaInsert, InlineRange } from '@blockexpanse/inline';
...
import { FONT_XS, PANEL_BASE } from '@blockexpanse/affine-shared/styles';
import { type BlockStdScope, ShadowlessElement } from '@blockexpanse/block-std';
import {
  assertExists,
  SignalWatcher,
  WithDisposable,
} from '@blockexpanse/global/utils';
import { DoneIcon, ResetIcon } from '@blockexpanse/icons/lit';
...
export class ReferenceAliasPopup extends SignalWatcher(
  ...
  override render() {
    ...
    const resetLabel = t(I18nKeys.editor.common.reset, 'Reset');
    const saveLabel = t(I18nKeys.editor.common.save, 'Save');
    return html`
      <div class="overlay-root">
        ...
        <div class="alias-form-popup">
          <input
            ...
            type="text"
            placeholder=${t(
              I18nKeys.editor.reference.addCustomTitle,
              'Add a custom title'
            )}
          ...
          >
            ...
            <span class="label">${saveLabel}</span>
          </editor-icon-button>
        </div>
      </div>
    `;
