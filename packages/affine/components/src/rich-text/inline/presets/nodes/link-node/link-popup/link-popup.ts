import type { EmbedOptions } from '@blockexpanse/affine-shared/types';
import type { InlineRange } from '@blockexpanse/inline/types';
...
import { toastI18n } from '../../../../../../toast/index.js';
import {
  renderActions,
...
export class LinkPopup extends WithDisposable(LitElement) {
    ...
    return html`
      <div class="affine-link-popover create">
        <input
          ...
          spellcheck="false"
          placeholder=${t(
            I18nKeys.editor.placeholder.pasteOrTypeLink,
      ...
      <div class="affine-link-edit-popover">
        <div class="affine-edit-area text">
          <input
            ...
            type="text"
            placeholder=${t(
              I18nKeys.editor.placeholder.enterText,
              'Enter text'
            )}
            @input=${this._updateConfirmBtn}
          />
          <label class="affine-edit-label" for="text-input"
            >${t(I18nKeys.editor.format.text, 'Text')}</label
          >
        </div>
        <div class="affine-edit-area link">
          ...
          <label class="affine-edit-label" for="link-input"
            >${t(I18nKeys.editor.format.link, 'Link')}</label
          >
        </div>
        ${this._confirmBtnTemplate()}
      </div>
  ...
  private _viewTemplate = () => {
    ...
    const buttons = [
      html`
        ...
        <editor-icon-button
          aria-label=${t(I18nKeys.editor.action.copy, 'Copy')}
          data-testid="copy-link"
          .tooltip=${t(I18nKeys.editor.action.copyLink, 'Copy link')}
          @click=${this._copyUrl}
        >
          ${CopyIcon}
        </editor-icon-button>
      ...
      html`
        <editor-menu-button
          ...
          .button=${html`
            <editor-icon-button
              aria-label=${t(I18nKeys.editor.action.more, 'More')}
              .tooltip=${t(I18nKeys.editor.action.more, 'More')}
            >
              ${MoreVerticalIcon}
            </editor-icon-button>
          `}
  ...
  private _moreActions() {
    ...
    return renderActions([
      [
        {
          label: t(I18nKeys.editor.action.open, 'Open'),
          type: 'open',
          icon: OpenIcon,
        ...
        {
          label: t(I18nKeys.editor.action.removeLink, 'Remove link'),
          type: 'remove-link',
          icon: UnlinkIcon,
  ...
  private _viewSelector() {
    ...
    buttons.push({
      type: 'inline',
      label: t(I18nKeys.editor.action.inlineView, 'Inline view'),
    });
    ...
    return html`
      <editor-menu-button
        ...
        .button=${html`
          ...
          >
            <div class="label">
              ${t(I18nKeys.editor.action.inlineView, 'Inline view')}
            </div>
            ${SmallArrowDownIcon}
          </editor-icon-button>
        `}
