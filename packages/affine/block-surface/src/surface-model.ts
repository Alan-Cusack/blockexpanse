import type { ConnectorElementModel } from '@blockexpanse/affine-model';
import type { SurfaceBlockProps } from '@blockexpanse/block-std/gfx';

import { SurfaceBlockModel as BaseSurfaceModel } from '@blockexpanse/block-std/gfx';
import { DisposableGroup } from '@blockexpanse/global/utils';
import { defineBlockSchema, DocCollection } from '@blockexpanse/store';
...
export class SurfaceBlockModel extends BaseSurfaceModel {
  ...
  override getElementsByType<
    K extends keyof BlockExpanse.SurfaceElementModelMap,
  >(type: K): BlockExpanse.SurfaceElementModelMap[K][] {
    return super.getElementsByType(
      type
    ) as BlockExpanse.SurfaceElementModelMap[K][];
  }
}
