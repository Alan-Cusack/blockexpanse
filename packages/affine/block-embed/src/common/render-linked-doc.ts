...
import { EMBED_CARD_HEIGHT } from '@blockexpanse/affine-shared/consts';
import { matchFlavours, SpecProvider } from '@blockexpanse/affine-shared/utils';
import { BlockStdScope } from '@blockexpanse/block-std';
...
async function renderNoteContent(
  ...
  const allowFlavours: (keyof BlockExpanse.BlockModels)[] = isHorizontal
    ? []
    : ['affine:image'];
