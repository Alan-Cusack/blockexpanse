import type { LinkPreviewData } from '@blockexpanse/affine-model';

import {
  getLinkPreviewEndpoint,
  setLinkPreviewEndpoint,
} from '@blockexpanse/affine-shared/services';
import { isAbortError } from '@blockexpanse/affine-shared/utils';
...
export class LinkPreviewer {
  /** Per-instance override; falls back to {@link getLinkPreviewEndpoint}. */
  private _endpoint = '';
  ...
  query = async (
    ...
    const endpoint = this._endpoint || getLinkPreviewEndpoint();
    if (!endpoint) {
      return {};
    }
  ...
  setEndpoint = (endpoint: string) => {
    ...
    // Keep the shared config in sync for other LinkPreviewer instances.
