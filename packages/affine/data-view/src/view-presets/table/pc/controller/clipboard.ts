...
const BLOCKEXPANSE_DATABASE_TABLE = 'blockexpanse/database/table';
...
export class TableClipboardController implements ReactiveController {
    ...
    this.clipboard
      .writeToClipboard(items => {
        return {
          ...items,
          [TEXT]: stringResult,
          [BLOCKEXPANSE_DATABASE_TABLE]: JSON.stringify(jsonResult),
        };
      })
      .then(() => {
        if (area[0]?.row) {
  ...
  private _onPaste = async (_context: UIEventStateContext) => {
    ...
    if (tableSelection) {
      ...
      const dataString = json[BLOCKEXPANSE_DATABASE_TABLE];
