...
import { persistViewName, t, tr } from '../../core/utils/i18n.js';
      ...
      [
        menu.group({
          items: views.map(id => {
            ...
            return menu.action({
              ...
              ></uni-lit>`,
              name: tr(view.name$.value ?? ''),
              label: () => html`${tr(view.name$.value ?? '')}`,
        ...
        menu.group({
          items: this.dataSource.viewMetas.map(v => {
            return menu.action({
              name: t()(
                I18nKeys.editor.dataView.view.createView,
                'Create {name}',
                { name: tr(v.model.defaultName) }
              ),
              hide: () => this.readonly,
        ...
        items: [
          ...
          menu.group({
            items: [
              ...
              menu.action({
                name: t()(I18nKeys.editor.dataView.view.moveLeft, 'Move Left'),
                hide: () => index === 0,
                prefix: MoveLeftIcon(),
                select: () => {
              ...
              menu.action({
                name: t()(
                  I18nKeys.editor.dataView.view.moveRight,
                  'Move Right'
                ),
  ...
  renderMore = (count: number) => {
    ...
    return html`
      <div class="database-view-button dv-hover" @click="${this._showMore}">
        ${t()(I18nKeys.editor.dataView.filterUI.more, '{count} More', {
    ...
    return views.map(id => () => {
      ...
      return html`
        ...
        >
          ...
          <div class="name">${tr(view?.name ?? '')}</div>
        </div>
      `;
    });
  };
