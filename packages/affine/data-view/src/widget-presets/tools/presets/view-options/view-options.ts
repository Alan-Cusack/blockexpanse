...
const createSettingMenus = (
  ...
  settingItems.push(
    menu.action({
      ...
      postfix: html` <div style="font-size: 14px;">
          ${t(I18nKeys.editor.dataView.property.shown, '{count} shown', {
            count: view.properties$.value.length,
          })}
        </div>
        ${ArrowRightSmallIcon()}`,
      select: () => {
  ...
  if (filterTrait) {
    ...
    settingItems.push(
      menu.action({
        name: t(I18nKeys.editor.dataView.filter, 'Filter'),
        prefix: FilterIcon(),
        postfix: html` <div style="font-size: 14px;">
            ${filterCount === 0
              ? ''
  ...
  if (sortTrait) {
    ...
    settingItems.push(
      menu.action({
        name: t(I18nKeys.editor.dataView.sort, 'Sort'),
        prefix: SortIcon(),
        postfix: html` <div style="font-size: 14px;">
            ${sortCount === 0
              ? ''
        ...
        select: () => {
          ...
          if (!sortList.length) {
            ...
            popSortRoot(target, {
              sortUtils: sortUtils,
              title: {
                text: t(I18nKeys.editor.dataView.sort, 'Sort'),
                onBack: reopen,
              },
            });
          }
  ...
  if (groupTrait) {
    settingItems.push(
      menu.action({
        name: t(I18nKeys.editor.dataView.group, 'Group'),
        prefix: GroupingIcon(),
        postfix: html` <div style="font-size: 14px;">
            ${tr(groupTrait.property$.value?.name$.value ?? '')}
          </div>
          ${ArrowRightSmallIcon()}`,
        select: () => {
...
export const popViewOptions = (
  ...
  items.push(
    menu.input({
      initialValue: tr(view.name$.value),
      onChange: text => {
        view.nameSet(persistViewName(text));
      },
    })
  );
  items.push(
    menu.group({
      items: [
        menu.action({
          ...
          postfix: html` <div style="font-size: 14px;">
              ${tr(
                view.manager.viewMetas.find(m => m.type === view.type)?.model
                  .defaultName ?? view.type
              )}
            </div>
            ${ArrowRightSmallIcon()}`,
          select: () => {
            const viewTypes = view.manager.viewMetas.map<MenuConfig>(meta => {
              return menu => {
                if (!menu.search(tr(meta.model.defaultName))) {
                  return;
                }
                const isSelected =
