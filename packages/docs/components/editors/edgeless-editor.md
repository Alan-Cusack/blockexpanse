...
<iframe src="https://try-blockexpanse.vercel.app/starter/?init&mode=edgeless" frameborder="no" width="100%" height="500"></iframe>
...
Moreover, this editor inherits capabilities built into the BlockExpanse framework, including:

- Per-user undo/redo stack
- Real-time collaboration
- [Document streaming](../../guide/data-synchronization#document-streaming)

Notably, the BlockExpanse framework allows runtime compatibility between the page editor and the edgeless editor, beyond mere static file format compatibility. This means you can dynamically attach the same doc object to different instances of the page editor and edgeless editor.

## Usage

```ts
import { EdgelessEditor } from '@blockexpanse/presets';
...
Like all BlockExpanse editors, the editor UI is entirely composed of the combination of [block specs](../../guide/block-spec). A specialized [root block](../blocks/root-block) spec serves as the root node of the document and implements all top-level document UI, with main widgets also mounted on the root block. Accordingly, commonly used editing APIs are provided in the root service.
...
- [`EdgelessEditor`](/api/@blockexpanse/presets/classes/EdgelessEditor.html)
- [`EdgelessRootService`](/api/@blockexpanse/blocks/classes/EdgelessRootService.html)
- [`SurfaceBlockModel`](/api/@blockexpanse/blocks/classes/SurfaceBlockModel.html)
- [`SurfaceBlockService`](/api/@blockexpanse/blocks/classes/SurfaceBlockService.html)
