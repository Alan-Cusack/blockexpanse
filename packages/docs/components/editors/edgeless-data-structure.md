...
In BlockExpanse, documents in edgeless mode are isomorphic to those in page mode. Edgeless documents are also composed of blocks, with no data conversion occurring during mode switching.
...
- The `block.children` field can contain edgeless-specific card blocks, such as embed-style links to YouTube, Figma, or other BlockExpanse documents.
...
BlockExpanse allows specific block types to appear in both locations. For example:
...
Instead, BlockExpanse allows specific block types to determine hierarchy order along with surface elements, including:
