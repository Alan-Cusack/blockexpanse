...
In BlockExpanse, all blocks should have a schema. The schema of the block describes the data structure of the block.
...
```ts
import { defineBlockSchema, type Text } from '@blockexpanse/store';
...
```ts
import { type SchemaToModel } from '@blockexpanse/store';
