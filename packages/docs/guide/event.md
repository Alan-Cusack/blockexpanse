...
This document introduces the handling of UI events, event flows within the block tree, and the implementation of hotkeys in BlockExpanse.

## Handling UI Events

For UI events such as `click` in editor, there is an underlying event dispatcher in `@blockexpanse/block-std` to dispatch events. With the dispatcher, you can handle events in your [block view](./block-view) implementation in this manner:
