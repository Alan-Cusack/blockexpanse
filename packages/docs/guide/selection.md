...
In BlockExpanse, we use a data driven approach to represent the selection. It also follows the [CRDT-native data flow](/blog/crdt-native-data-flow), which means the selection state is always derived from serializable data.
...
```ts
import { BaseSelection, PathFinder } from '@blockexpanse/block-std';
import z from 'zod';
